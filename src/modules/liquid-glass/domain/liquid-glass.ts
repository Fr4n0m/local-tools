...
const liquidGlassInputSchema = z.object({
  opacity: z.coerce.number().finite().catch(0.28),
  blur: z.coerce.number().finite().catch(18),
});
...
export function buildLiquidGlassCss(opacity: number, blur: number): string {
  const parsed = liquidGlassInputSchema.parse({ opacity, blur });
  const safeOpacity = clampGlassOpacity(parsed.opacity);
  const safeBlur = Math.max(4, Math.min(40, Math.round(parsed.blur)));
